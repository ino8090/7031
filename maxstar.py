...
STREAM_KEY = os.getenv("STREAM_KEY", "maxstar")
RTMP_SERVER = f"{RTMP_URL}/{STREAM_KEY}"

M3U_URL = os.getenv("M3U_URL", "https://raw.githubusercontent.com/ino8090/0101/refs/heads/main/yerli.m3u")
LOGO_URL = os.getenv("LOGO_URL", "https://raw.githubusercontent.com/ino8090/0101/refs/heads/main/1787671807314.png")
...
# GitHub Actions job'ı 6 saat (360 dk) sonra zorla kesiyor.
# Script kendini bundan biraz ÖNCE düzgünce durdursun ki:
#  - Son konum Gist'e sağlıklı kaydedilsin
#  - FFmpeg temiz kapansın
#  - Workflow'un son adımı çalışıp yeni run'ı tetikleyebilsin
# Varsayılan: 5 saat 45 dakika (20700 sn) -> 15 dakikalık güvenlik payı bırakır.
MAX_RUNTIME_SECONDS = int(os.getenv("MAX_RUNTIME_SECONDS", 20700))
...
def stop_ffmpeg_gracefully(process):
    """FFmpeg'i düzgünce durdurmayı dener, olmazsa zorla kapatır."""
    try:
        process.terminate()
        process.wait(timeout=15)
    except subprocess.TimeoutExpired:
        print("⚠️ FFmpeg zamanında kapanmadı, zorla sonlandırılıyor.")
        process.kill()
    except Exception as e:
        print(f"⚠️ FFmpeg kapatma hatası: {e}")
...
def start_m3u_stream():
    ...
    print(f"🔧 Maksimum çalışma süresi: {format_hms(MAX_RUNTIME_SECONDS)} (sonra kontrollü yeniden başlatma tetiklenecek)")
    ...
    while True:
        # ---- Süre doldu mu kontrolü (yeni yayın döngüsüne girmeden önce) ----
        if time.time() - run_start_time > MAX_RUNTIME_SECONDS:
            print("⏰ Maksimum çalışma süresine ulaşıldı, script kontrollü şekilde sonlanıyor.")
            write_step_summary("—", current_index, 1, last_seconds, status="🔁 Süre doldu, yeni run tetiklenecek")
            return
        ...
        while True:
            # ---- Süre doldu mu kontrolü (yayın sırasında da kontrol ediyoruz) ----
            if time.time() - run_start_time > MAX_RUNTIME_SECONDS:
                print("⏰ Maksimum çalışma süresine ulaşıldı, FFmpeg düzgünce durduruluyor...")
                stop_ffmpeg_gracefully(process)
                update_gist_state(current_index, current_stream_seconds)
                write_step_summary(film_title, current_index, len(playlist), current_stream_seconds, status="🔁 Süre doldu, yeni run tetiklenecek")
                print("✅ Konum kaydedildi. Script sonlandırılıyor -> workflow'un restart adımı devreye girecek.")
                return
...
if __name__ == "__main__":
    start_m3u_stream()
    print("🏁 Script normal şekilde sona erdi.")
